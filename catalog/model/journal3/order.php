...
class ModelJournal3Order extends Model {
	...
	public function load($order_id) {
		...
		$order_data['secondary_telephone'] = '';
		...
		if ($this->customer->isLogged()) {
			...
			if ($order_info) {
				...
				$order_data['secondary_telephone'] = $order_info['secondary_telephone'];
	...
	public function save($order_id, $order_data) {
		...
		if (!$this->customer->isLogged() && isset($this->request->post['order_data'])) {
			...
			$order_data['secondary_telephone'] = Arr::get($this->request->post, 'order_data.secondary_telephone', '');
		...
		if ($this->journal3->isOC2()) {
			$this->db->query("
				...
				SET 
					...
					secondary_telephone = '" . $this->db->escape($data['secondary_telephone']) . "', 
...
// 		echo "<pre>";
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$lots_query_result = $this->db->query("SELECT inventory_details_id FROM " . DB_PREFIX . "inventory_details WHERE product_id = " . $product['product_id'] . " AND current_quantity > 0 AND is_merge_lot_quantity_to_main = 1 LIMIT 1")->row;
// 		print_r($lots_query_result);
				
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', lot_id = '". (int)$lots_query_result['inventory_details_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");
	...
	private function getOrder($order_id) {
		...
		if ($order_query->num_rows) {
			return array(
				...
				'secondary_telephone'     => $order_query->row['secondary_telephone'],
