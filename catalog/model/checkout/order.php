		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				foreach ($order_products as $order_product) {
                    ...
                    //start code for auto stock staus change with quantity
                    ...
                    $this->db->query("
                                    ...
                                    SET stock_status_id = '" . (int)$stock_status_id . "' 
                                    WHERE product_id = '" . (int)$order_product['product_id'] . "'
                                    ");
                    //end code for auto stock staus change with quantity
                    
                    // $this->db->query("UPDATE `" . DB_PREFIX . "inventory_details` SET current_quantity = (current_quantity - " . (int)$order_product['quantity'] . ") WHERE inventory_details_id = '" . (int)$order_product['lot_id']);
                    $this->db->query("UPDATE `" . DB_PREFIX . "inventory_details` 
                                      SET current_quantity = current_quantity - " . (int)$order_product['quantity'] . " 
                                      WHERE inventory_details_id = '" . (int)$order_product['lot_id'] . "'");


                    //check lot id with order_id and product from order_product table
                    // $check = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_product WHERE order_id = " . (int)$order_product['quantity'] . " AND product_id = " . (int)$order_product['quantity'] . ")
