...
class ControllerJournal3Checkout extends \Journal3\Opencart\Controller {
	...
	public function index() {
        ...
        $data['checkout_data']['order_data']['secondary_telephone'] = Arr::get($this->session->data, 'order_data.secondary_telephone', '');
	...
	public function save() {
        ...
        if (isset($this->request->post['order_data']['secondary_telephone'])) {
            $this->session->data['order_data']['secondary_telephone'] = $this->request->post['order_data']['secondary_telephone'];
        }
		...
		if ($this->checkCart()) {
			...
			if (Arr::get($this->request->get, 'confirm') === 'true') {
				...
				if (!$error) {
                    ...
                    if ($this->customer->isLogged()) {
                        $this->load->model('account/customer');
                        $this->model_account_customer->syncCustomerData($order_data);

						// Save address to account so next checkout auto-fills.
						// Journal3 can keep payment_address_type as "existing" even when user edits fields,
						// so we ensure the address book gets created/updated on confirm.
						$has_any_address = (int)$this->model_account_address->getTotalAddresses() > 0;
						$force_default = !$has_any_address || !$this->customer->getAddressId();

						$this->addAccountAddress('payment', $force_default);

						if ($this->cart->hasShipping() && !Arr::get($this->session->data, 'same_address')) {
							$this->addAccountAddress('shipping', false);
						}

						// Refresh checkout customer context (addresses, defaults)
						$this->model_journal3_checkout->editCustomerId($this->customer->getId());
                    } else {
                        $this->load->model('account/customer');
                        $customer_id = $this->model_account_customer->syncCustomerData($order_data);
                        
                        $this->customer->login($order_data['telephone'], $order_data['telephone']);

						// After auto-register + auto-login, also persist addresses to address book.
						if ($this->customer->isLogged()) {
							$has_any_address = (int)$this->model_account_address->getTotalAddresses() > 0;
							$force_default = !$has_any_address || !$this->customer->getAddressId();
						...
						// If an order was already created as guest, attach it to the new customer
						// so it appears in order history immediately after auto-login.
						if (!empty($this->session->data['order_id']) && $this->customer->isLogged()) {
							$this->db->query("UPDATE `" . DB_PREFIX . "order` SET customer_id = '" . (int)$this->customer->getId() . "', customer_group_id = '" . (int)$this->customer->getGroupId() . "' WHERE order_id = '" . (int)$this->session->data['order_id'] . "' AND customer_id = '0'");
						}
                    }
	...
	private function getCheckoutData($data) {
		...
		return array(
			...
			'is_coupon_applied'          => isset($this->session->data['coupon']) && $this->session->data['coupon'] ? true : false,
...
private function validateAccount($custom_fields) {
    ...
    $secondary_telephone = Arr::get($this->request->post, 'order_data.secondary_telephone');
    $email = Arr::get($this->request->post, 'order_data.email');
    ...
    $pattern = '/^01[3456789][0-9]{8}$/';
    if (!preg_match($pattern, $telephone)) {
        $error['telephone'] = "Invalid Number! Must be 11 digits and start with 01.";
    } else {
        $this->load->model('account/customer');
        if (method_exists($this->model_account_customer, 'getTotalCustomersByTelephone')) {
            $customer_total = $this->model_account_customer->getTotalCustomersByTelephone($telephone);
            if ($customer_total) {
                $error['telephone'] = "This telephone number is already registered!";
            }
        }
    }
    
    if (!empty($secondary_telephone)) {
        if (!preg_match($pattern, $secondary_telephone)) {
            $error['secondary_telephone'] = "Invalid Number! Must be 11 digits and start with 01.";
        } else {
            if ($secondary_telephone == $telephone) {
                $error['secondary_telephone'] = "Secondary number cannot be the same as the primary number!";
            }
        }
    }
    ...
    // if ((utf8_strlen($telephone) < 1) || (utf8_strlen($telephone) > 3)) {
    //     $error['telephone'] = $this->language->get('error_telephone');
    // } else {
    //     $this->load->model('account/customer');
    //     if (method_exists($this->model_account_customer, 'getTotalCustomersByTelephone')) {
	...
	private function registerGuest($order_data) {
		...
		foreach ($order_data as $key => $value) {
			...
			} else if (in_array($key, array('firstname', 'lastname', 'email', 'telephone', 'secondary_telephone', 'fax', 'custom_field', 'customer_group_id'))) {
	...
	private function registerAccount($order_data) {
		...
		foreach ($order_data as $key => $value) {
			...
			if (Str::startsWith($key, 'payment_')) {
				...
				$shipping_address[str_replace('shipping_', '', $key)] = $value;
			} else if (in_array($key, array('firstname', 'lastname', 'email', 'telephone', 'secondary_telephone', 'fax', 'custom_field', 'customer_group_id', 'password', 'company'))) {
				$account_data[$key] = $value;
			}
		}
	...
	private function addAccountAddress($type, $force_default = false) {
		$address = $this->model_journal3_checkout->extract_address($type, $this->model_journal3_checkout->get_address($type));

		// Make sure required keys exist (prevents undefined index notices and empty inserts)
		$address = array_replace(array(
			'firstname'    => '',
			'lastname'     => '',
			'company'      => '',
			'address_id'   => '',
		...
		if ($this->journal3->isOC2()) {
			...
			$existing_address_id = (int)$address['address_id'];
			$is_existing_selected = Arr::get($this->session->data, $type . '_address_type') === 'existing' && $existing_address_id > 0;
			...
			if ($is_existing_selected) {
				$this->model_account_address->editAddress($existing_address_id, $address);
