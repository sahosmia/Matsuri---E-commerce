...
class ModelAccountCustomer extends Model {
    ...
    public function getCustomerByEmailOrPhone($login_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer WHERE (LOWER(email) = '" . $this->db->escape(utf8_strtolower($login_id)) . "' OR telephone = '" . $this->db->escape($login_id) . "') AND status = '1'");
        return $query->row;
    }
...
public function syncCustomerData($data) {
    ...
    $query = $this->db->query("SELECT customer_id FROM " . DB_PREFIX . "customer WHERE telephone = '" . $telephone . "'");

    if ($query->num_rows) {
        $customer_id = $query->row['customer_id'];
        $this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $telephone . "', status = '1' WHERE customer_id = '" . (int)$customer_id . "'");
        return $customer_id;
    } else {
        $salt = token(9);
        $password = $data['telephone']; 
        $hashed_password = sha1($salt . sha1($salt . sha1($password)));

        $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "', store_id = '" . (int)$this->config->get('config_store_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $telephone . "', salt = '" . $this->db->escape($salt) . "', password = '" . $this->db->escape($hashed_password) . "', status = '1', date_added = NOW()");
