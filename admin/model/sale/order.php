...
class ModelSaleOrder extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS order_status, o.shipping_code, o.total, o.shipping_address_1, o.telephone, o.secondary_telephone, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
		...
		if (!empty($data['filter_from_date']) && !empty($data['filter_to_date'])) {
			$sql .= " AND DATE(o.date_added) BETWEEN DATE('" . $this->db->escape($data['filter_from_date']) . "') AND DATE('" . $this->db->escape($data['filter_to_date']) . "')";
		}elseif(!empty($data['filter_from_date'])){
			$sql .= " AND DATE(o.date_added) > DATE('" . $this->db->escape($data['filter_from_date']) . "')";
		}elseif(!empty($data['filter_to_date'])){
			$sql .= " AND DATE(o.date_added) < DATE('" . $this->db->escape($data['filter_to_date']) . "')";
		}
	...
	public function getTotalOrderss($data = array()) {
