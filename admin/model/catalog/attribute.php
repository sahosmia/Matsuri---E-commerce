...
class ModelCatalogAttribute extends Model {
	...
	public function getTotalAttributes() {
		...
		if (!empty($this->request->get['filter_name'])) {
			...
			$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "attribute cp INNER JOIN " . DB_PREFIX . "attribute_description cd2 ON (cp.attribute_id = cd2.attribute_id) WHERE cd2.name LIKE '" . $this->db->escape($filter_name) . "%'";
		} else {
			$sql = "SELECT COUNT(*) AS total FROM " . DB_PREFIX . "attribute";
		}
		$query = $this->db->query($sql);
