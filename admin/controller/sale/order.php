...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'secondary_telephone'      => $result['secondary_telephone'],
	...
	public function getForm() {
		...
		if (!empty($order_info)) {
			...
			foreach ($products as $product) {
				$product_row = $this->model_catalog_product->getProduct((int)$product['product_id']);
				$sku = ($product_row && isset($product_row['sku'])) ? $product_row['sku'] : '';
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['email'] = $order_info['email'];
			$data['telephone'] = $order_info['telephone'];
			$data['secondary_telephone'] = $order_info['secondary_telephone'];
...
// 			$data['payment_address'] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));
			...
			foreach ($products as $product) {
				...
				$lot_current_stock = null;
				$lots_query_results = array();

				if (!empty($product['lot_id'])) {
					$lots_query_results = $this->model_catalog_product->getLotByLotId($product['lot_id']);
					if ($lots_query_results) {
						$lot_current_stock = isset($lots_query_results['current_quantity']) ? $lots_query_results['current_quantity'] : null;
					}
				}

				$lot_display_name = (isset($lots_query_results['lot_number'])) ? $lots_query_results['lot_number'] : '';
				...
				$data['products'][] = array(
					...
					'model'    		   => $product['model'],
					'sku'              => $sku,
					'lot'              => $lot_display_name,
					'lot_current_stock'=> $lot_current_stock,
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				$data['orders'][] = array(
					...
					'secondary_telephone'  => $order_info['secondary_telephone'],
